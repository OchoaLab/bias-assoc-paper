...
Given a desired number of causal loci $m_1$ and heritability $h^2=0.8$, the goal is to choose causal coefficients $\boldsymbol{\beta}$ and the intercept $\alpha$ that result in zero mean and the desired trait heritability.
...
To compare the results from different kinship estimators on a real dataset, we use genotypes from the 1000 Genomes Project \citep{the_1000_genomes_project_consortium_map_2010, 1000_genomes_project_consortium_integrated_2012} but simulate traits based on the real genotypes with the number of causal loci equals to 250. The dataset was processed with \texttt{plink2} \citep{chang_second-generation_2015} and R packages \texttt{genio} and \texttt{BEDMatrix} \citep{grueneberg_bgdata_2019}. Only data with autosomal biallelic SNP loci filter ``PASS'' are included in the analysis.
Moreover, we use \texttt{plink2} parameters ``\texttt{-{}-indep-pairwise 1000kb 0.3}'' to further iteratively removes loci that have a greater than 0.3 correlation coefficient with another locus that is within 1000kb. We also remove all loci with $\text{MAF} < 0.01$ in this real dataset.

\subsection{Evaluation of performance}

$\auc$ and $\rmsd$ are used to evaluate each approaches. For the later one, $\rmsd$ (Signed Root Mean Square Deviation) is used to measure the difference between the observed null p-value quantiles and the expected uniform quantiles (p-values of continuous test statistics follow a uniform distribution under the null). The expression for $\rmsd$ is:
$$
\rmsd
=
...
To compare performance of different estimates on a real dataset, we use the genotype from the 1000 Genomes Project but simulate traits using 250 causal loci. The kinship estimates are shown in \cref{fig:kinship_real}. The popkin ROM estimates shows a tree structure that is aligned with relationships of different subpopulations in the 1000 Genomes Project (\cref{fig:kinship_real}A), while the popkin MOR estimates provide similar patterns but do not have such tree structure (\cref{fig:kinship_real}B). The standard kinship estimators (ROM and MOR) generally downwardly biased especially among EUR,SAS,EAS and AMR subpopulations (\cref{fig:kinship_real}C-D). For the Weir-Goudet estimates, they uniformally provide a smaller values (\cref{fig:kinship_real}E-F).

When we perform PCA and LMM association test using these kinship estimators, the results are similar to our simulation study with higher variances: LMM perform better than PCA while there are not much difference among kinship estimators used within the same association model (\cref{fig:auc_real}, \cref{fig:rmsd_real}). The MOR estimates using LMM slightly outperform ROM estimates using LMM for both $\auc$ and$\rmsd$.
We also measure the correlation between p-values of different association models and kinship estimates. They are all strongly correlated based on this 1000 Genomes Project real dataset with most achieved correlation of 1 (\cref{fig:pvals_cor_real}).
