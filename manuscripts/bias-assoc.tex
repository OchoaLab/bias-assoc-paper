...
%  \frac{ \mathbf{1}^\intercal \left( \kinMat \right)^{-1} }{ \mathbf{1}^\intercal \left( \kinMat \right)^{-1} \mathbf{1} } \kinMat \frac{ \left( \kinMat \right)^{-1} \mathbf{1} }{ \mathbf{1}^\intercal \left( \kinMat \right)^{-1} \mathbf{1} }
  ...
  We found empirically that WG estimates are non-positive semidefinite, and that this is due to a single eigenvalue that is negative.
  We did not find a closed form for this eigenvector, but we prove that it must have non-zero projections onto two orthogonal subspaces: $S_1$ spanned by the intercept $\mathbf{1}$, and $S_2$ its complement (orthogonal to $\mathbf{1}$).
  This is because, starting from a positive definite kinship matrix \kinMat, the WG-bias transformed matrix $\kinMatPrime = F^\text{WG}( \kinMat )$ is positive definite in each of the subspaces $S_1, S_2$ (shown below), so the eigenvector with the negative eigenvalue cannot be wholly in just one of the two subspaces.
  ...
  .
  $
  The transformation requires $\tilde{\varphi}^T < 1$, so $\kinMat \ne \mathbf{J}$.
  ...
  ,
  $$
  which follows since the original kinship matrix is positive definite and $1-\tilde{\varphi}^T > 0$.
  ...
  ,
  $$
  where $\bar{\varphi}^T$ is the overall mean kinship value, while $\tilde{\varphi}^T$ is the mean of the off-diagonal kinship values only (\cref{eq:wg_tilde}), which follows since $v^2, n^2, 1 - \tilde{\varphi}^T > 0$, and $\tilde{\varphi}^T < \bar{\varphi}^T$ (\cref{sec:mean_kinship_ineqs}; note $\tilde{\varphi}^T = \bar{\varphi}^T$ occurs if and only if $\kinMat = \bar{\varphi}^T \mathbf{J}$, which is not positive definite).

  Lastly, consider the optimal weights from \cref{sec:min_w_mean_kin}, calculated for the unbiased kinship matrix \kinMat:
  $$
  \mathbf{w}
  = 
  \frac{ \left( \kinMat \right)^{-1} \mathbf{1} }{ \mathbf{1}^\intercal \left( \kinMat \right)^{-1} \mathbf{1} }
  .
  $$
  When applied to \kinMatPrime, since $\mathbf{w}^\intercal \mathbf{1} = 1$, we get
  \begin{align*}
    ...
    &=
      ...
      \right)
  \end{align*}
  Thus, we again find that a WG-biased matrix is non-positive semidefinite if $1 / \left( \mathbf{1}^\intercal \left( \kinMat \right)^{-1} \mathbf{1} \right) \le \tilde{\varphi}^T$, which empirically is always true in our evaluations.
  This $\mathbf{w}$ is not an eigenvector (even if normalized), but empirically it has a large projection to the desired eigenvector with the negative eigenvalue.
