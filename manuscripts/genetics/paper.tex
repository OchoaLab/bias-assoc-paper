...
% Author added:
% cleveref only for equations and (appendix) sections; figures needed hacks to accomodate supplementary cases
...
\runningtitle{Association robust to kinship bias} % For use in the footer
\runningauthor{Hou \textit{et al.}}
...
\correspondingauthoraffiliation[$\ast$]{Corresponding author: \texttt{alejandro.ochoa@duke.edu}}
...
% hack to define supplementary references without including actual figures/etc
\makeatletter
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{kinship_sim.pdf}
  \caption{
...
% kinship matrices
Kinship estimates and limits for this simulation are shown in Fig. \ref{fig:kinship_sim}.
...
Only Popkin ROM is unbiased, while popkin MOR has a slight upward bias that varies across the matrix (Fig. \ref*{fig:popkin-rom-mor}A).
In contrast, the Standard and Weir-Goudet (WG) estimates have large downward biases overall, resulting in abundant negative values; Standard biases vary for every pair of individuals (as described in \cref{eq:kinship_std_lim}; Fig. \ref*{fig:kinship-bias}A), while WG has a uniform bias (following \cref{eq:wg_lim}).
The difference is most noticeable near the diagonal: the true kinship matrix has monotonically increasing values, WG has smaller values but which are still monotonically increasing, and Standard estimates follow a U-shaped pattern (decreasing at first, then increasing again in Fig. \ref{fig:kinship_sim}D-F).

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{auc_sim.pdf}
  \caption{
...
Surprisingly, we find that kinship bias type does not have a discernible effect on association performance, as summarized by $\auc$ (a robust proxy for power; high and low heritability in Figs. \ref{fig:auc_sim}, \ref*{fig:auc_sim-h3}, respectively) and $\rmsd$ (measures null statistic calibration; Figs. \ref*{fig:rmsd_sim}, \ref*{fig:rmsd_sim-h3}).
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{pvals_eq_sim.pdf}
  \caption{
...
We measure high correlations $\rho$ between p-values, near $\rho = 1$ for comparisons involving the same model (between LMM methods or between PCA methods, both heritabilities), and across models around $\rho = 0.6$ for $h^2=0.8$, which increases to $\rho = 0.88$ for $h^2=0.3$ (Figs. \ref*{fig:pvals_cor_sim}, \ref*{fig:pvals_cor_sim-h3}).
To measure numerical agreement more stringently, we calculate the proportion of loci between two methods with p-values within 0.01 of each other, and find a remarkably high agreement between estimators of different bias types after matching association model and locus weight type or limit (Figs. \ref{fig:pvals_eq_sim}, \ref*{fig:pvals_eq_sim-h3}).
...
Kinship estimates are shown in Fig. \ref{fig:kinship_real} (note real data have no true kinship or estimator limits).
Popkin ROM estimates display an approximate nested block structure that arises from the tree relationships between subpopulations (Fig. \ref{fig:kinship_real}A; trees were explicitly fit to this data in previous work \citep{yao_limitations_2022}).
However, popkin MOR estimates do not follow the nested blocks tree structure, since kinship between African and non-African populations is higher than kinship within African populations (Figs. \ref{fig:kinship_real}B, \ref*{fig:popkin-rom-mor}B).
Standard estimates have values closer to zero, and a different bias for each pair of individuals (Fig. \ref*{fig:kinship-bias}B), resulting in higher relative kinship for African compared to non-African populations (Fig. \ref{fig:kinship_real}C-D), whereas kinship in African populations is the lowest in the unbiased estimate (Fig. \ref{fig:kinship_real}A).
Lastly, WG estimates are uniformly smaller than popkin's and attain large negative values (Fig. \ref{fig:kinship_real}E-F).

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{kinship_real.pdf}
  \caption{
    {\bf Kinship estimates on 1000 Genomes.}
    Each panel represents a kinship matrix as a heatmap, as in Fig. \ref{fig:kinship_sim}.
...
However, unlike the simulation, here for $h^2=0.8$ (but not 0.3) the MOR estimates noticeably outperform ROM estimates (LMM only), in terms of both $\auc$ (Figs. \ref{fig:auc_real}, \ref*{fig:auc_real-h3}) and $\rmsd$ (Figs. \ref*{fig:rmsd_real}, \ref*{fig:rmsd_real-h3}).
% agreement
P-values are even more highly correlated in this case (Figs. \ref*{fig:pvals_cor_real}, \ref*{fig:pvals_cor_real-h3}), and again nearly identical at a large proportion of loci between approaches with matched association model and locus weight type (MOR or ROM), regardless of bias type (Figs. \ref*{fig:pvals_eq_real}, \ref*{fig:pvals_eq_real-h3}).

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{auc_real.pdf}
  \caption{
...
The Wald test used by GCTA \citep{yang_advantages_2014} is also invariant to these kinship biases given our empirical results in Figs. \ref{fig:pvals_eq_sim}, \ref*{fig:pvals_eq_real}, and proven explicitly for WG bias in \cref{sec:wg_gls}.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{pcs.pdf}
  \caption{
...
We visualize the top PCs of our datasets in Fig. \ref{fig:pcs} to assess the validity of \cref{eq:pc-shift}.
...
Fig. \ref{fig:pcs} indeed shows that PC1 is shifted by noticeable amounts in each of these cases, while PC2 is less shifted.
...
Overall, while the approximation of Fig. \ref{eq:pc-shift} can be weakened to merely require that the biased PCs plus intercept span the same subspace of the unbiased PCs plus intercept, the approximate PC shifts better explain intuitively why the result for LMM is also observed for PCA association.
...
We find that all WG matrices have very large negative minimum eigenvalues, and popkin MOR estimates also have smaller negative minimum eigenvalues (Figs. \ref*{fig:emin}, \ref*{fig:emin-h3}).
Moreover, besides all WG matrices and most popkin MOR estimates, Standard matrices are also often non-PSD but only in 1000 Genomes (Figs. \ref{fig:emin-cut}, \ref*{fig:emin-cut-h3}), which has missing genotypes (the admixed family simulation does not have missing genotypes).
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{emin-cut.pdf}
  \caption{
...
As expected, we see that Standard kinship matrices are singular on our admixed family simulation (which lacks missingness), as reflected by extremely high condition numbers, but their trait covariances have small condition numbers (Figs. \ref*{fig:kappa}, \ref*{fig:kappa-h3}).
...
We see that popkin MOR estimates are non-PSD (Fig. \ref*{fig:emin}) in such a way that some of their $\mathbf{V}$ are ill-conditioned under high heritability (Fig. \ref*{fig:kappa}) but not the lower heritability (Fig. \ref*{fig:kappa-h3}), and this explains its poorer performance in the admixed family evaluations with high heritability (Figs. \ref{fig:auc_sim}, \ref*{fig:rmsd_sim}), as shown in the next subsection.
...
With PCA, no deviations from expectation of $\auc$ and $\rmsd$ are observed for WG (Figs. \ref{fig:auc_sim}, \ref{fig:auc_real}, \ref*{fig:rmsd_sim}, \ref*{fig:rmsd_real}), which makes sense since PCA simply ignores eigenvectors with negative or zero eigenvalues.
...
Thus, the relationship between WG, Standard, and True or Popkin variance components are largely as expected from our theoretical prediction $\sigma^{2\prime} = c \sigma^2$ in \cref{eq:sigma-rescale}, with the exception of popkin ROM on 1000 Genomes $h^2=0.8$ only, whose genetic variance estimates are slightly smaller than expected (Figs. \ref*{fig:preds-reml-errors}, \ref*{fig:preds-reml-errors-h3}).
...
We find that the small performance errors of popkin ROM in 1000 Genomes (Figs. \ref{fig:auc_real}, \ref*{fig:rmsd_real}) are driven by errors in genetic variance component estimation $\sigma^2$ (Fig. \ref*{fig:reml-err-vs-pred-err}).
However, the larger performance errors of popkin MOR in the admixed family simulation (Figs. \ref{fig:auc_sim}, \ref*{fig:rmsd_sim}) are instead explained by the condition number of $\mathbf{V}$ (Fig. \ref*{fig:kappa-vs-pred-err}).
...
Previous research showed that commonly used kinship estimators are biased, and that these biases can be large (\citet{ochoa_estimating_2021}; Figs. \ref{fig:kinship_sim}, \ref*{fig:kinship-bias}).
...
Well-conditioned association is guaranteed for PSD kinship matrices, and popkin ROM is the only estimator that produces PSD matrices consistently across our evaluations (Fig. \ref{fig:emin-cut}).
...
We also observe smaller numerical inaccuracies for popkin ROM, the estimator we recommend, in 1000 Genomes with $h^2=0.8$ only, although the result is mixed: performance is slightly better (Fig. \ref{fig:auc_real}) although null p-value calibration is slightly worse (Fig. \ref*{fig:rmsd_real}).
The cause is variance components are poorly estimated (Fig. \ref*{fig:preds-reml-errors}), but we did not find a more fundamental explanation.
...
% MOR weirdness
Despite being non-PSD, we observe better performance for MOR versus ROM estimators in LMM association of 1000 Genomes with $h^2=0.8$ (Fig. \ref{fig:auc_real}; the approaches were tied under lower heritability in Fig. \ref*{fig:auc_real-h3}).
...
This effect is not observed in the admixed family simulation, where MOR and ROM versions give similar kinship estimates (Fig. \ref{fig:kinship_sim}) and performed similarly (Fig. \ref{fig:auc_sim}), compared to 1000 Genomes where kinship estimates are also strikingly different (Fig. \ref{fig:kinship_real}).
However, only popkin ROM is unbiased (Figs. \ref{fig:kinship_sim}B, \ref*{fig:popkin-rom-mor}).
...
\begin{appendices}
  ...
  % allow counting here! (counteracts setting in `gsajnl.cls`)
...
% define labels for supplementary figures, which are not actually present in this document
% https://tex.stackexchange.com/questions/582054/how-to-make-fake-labels
