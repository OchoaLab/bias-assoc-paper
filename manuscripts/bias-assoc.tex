...
$
However, the actual $\tilde{\varphi}^T$ does not satisfy this inequality in any of our empirical cases, so both values of $b$ above are complex.
This is a consequence of WG estimates being non-positive definite, which we elaborate in the following sections.
Nevertheless, we find that in practice inference with LMMs works even for non-positive definite kinship matrices, and the same theoretical result applies to these cases except without invoking complex numbers.
...
One last detail worth noting is that PCs can change order upon the bias transformation, which we noticed in the simulation, where PC2 and PC3 from popkin (and true kinship) actually correspond to PC1 and PC2, respectively, in both Standard and Weir-Goudet estimates (both ROM and MOR), and were plotted as such.
...
\subsection{Characterization of non-positive definite and singular kinship and trait covariance estimators}

While attempting to validate and characterize the earlier factorization of the WG bias function (\cref{eq:kin-bias-general,eq:B-shift,eq:wg-fac-b,eq:wg-fac-q}), we discovered that this estimator does not produce positive definite matrices, which covariance matrices are required to be.
To characterize this problem more generally, we calculated the eigenvalues of all of the kinship matrices we produced in all evaluations $\kinMat$ as well as the trait covariance matrices given by $\mathbf{V} = 2 \sigma^2 \kinMat + \sigma^2_\epsilon \mathbf{I}$, which we obtained using GCTA's estimates of $\sigma^2$ and $\sigma^2_\epsilon$.

We found that all WG matrices have very large negative minimum eigenvalues, and popkin MOR estimates also have smaller negative eigenvalues (\cref{fig:emin}).
...
$$
where $\mathbf{U}$ and $\mathbf{\Lambda}$ are the eigenvectors and eigenvalues of \kinMat (\cref{eq:kin_evd}),
so the eigenvectors of $\mathbf{V}$ are also $\mathbf{U}$ and its eigenvalues are the diagonal values of $2 \sigma^2 \mathbf{\Lambda} + \sigma^2_\epsilon \mathbf{I}$.
...
This equation also shows that $\mathbf{V}$ is always invertible and well-conditioned even when \kinMat is singular positive semi-definite, as the Standard estimator is under no missingness, since a zero kinship eigenvalue becomes $\sigma^2_\epsilon$ for $\mathbf{V}$.
Conversely, the above equation explains why some non-positive definite kinship matrices are particularly problematic: small negative eigenvectors with values near $-\sigma^2_\epsilon/(2\sigma^2)$ can result in ill-conditioned $\mathbf{V}$.
We see that popkin MOR estimates are non-positive definite (\cref{fig:emin}) in such a way that their $\mathbf{V}$ are ill-conditioned (\cref{fig:kappa}), and we find that this explains its poorer performance in the admixed family evaluations (\cref{fig:auc_sim,fig:rmsd_sim}), as shown in the next subsection.

\subsection{Further empirical validation of theoretical predictions}

Seeing that WG is always non-positive definite, and to query other instances where predictions were not fully met, here we analyze estimation accuracy of various parameters to better understand theoretically and empirically how this broken assumption affects them.
...
In this second step of association testing, GCTA and other LMMs use generalized least squares to estimate fixed effects coefficients once $\mathbf{V}$ is determined \citep{kang_efficient_2008, kang_variance_2010, yang_advantages_2014}.
...
We then compare intercept coefficients, which are not given by GCTA, and confirmed our theoretical prediction that they are identical whether the True or WG ROM limit kinship matrices are used (the mean absolute difference was below $10^{-7}$).
...
\begin{figure}[bp!]
  ...
  \caption{
    ...
    All WG matrices has very large negative eigenvalues, and Popkin MOR has negative eigenvalues as well; in these cases a non-positive definite kinship matrix always resulted in a non-positive definite $\mathbf{V}$.
  }
  \label{fig:emin}
\end{figure}
