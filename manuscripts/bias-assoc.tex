...
\begin{equation}
  ...
  \end{split}
\end{equation}
In \cref{sec:wg_posdef} we prove that $F^\text{WG}( \kinMat )$ is positive definite if \kinMat also is.
...
$\sigma^2$ is the random effect variance component factor,
...
Briefly, first $m_1$ causal loci are randomly selected, and $\mathbf{X}$ is subset to these loci and reindexed.
...
Our empirical observations suggested that replacing a kinship matrix with either the Standard- or WG-biased version does not alter association statistics; here prove a general version of these facts mathematically.
...
In all these cases $\mathbf{B}$ and $\kinMat'$ are singular transformations, since $\mathbf{B} \mathbf{1} = \mathbf{0}$ and $\mathbf{B}^\intercal \mathbf{b} = \mathbf{0}$.
...
\begin{align*}
  \mathbf{b}
  &=
    b \frac{ \kinMat^{-1} \mathbf{1} }{ \mathbf{1}^\intercal \kinMat^{-1} \mathbf{1} }
  , \\
  b
  &=
    1 \pm \sqrt{1 - (\mathbf{1}^\intercal \kinMat^{-1} \mathbf{1}) \tilde{\varphi}}
.
\end{align*}
The determinant of $b$ is non-negative, since
$
\tilde{\varphi} \le 1 / (\mathbf{1}^\intercal \kinMat^{-1} \mathbf{1})
$
[TODO: prove it!], so both values of $b$ above are real and valid, the positive root being positive while the negative one is negative.
...
(This $\mathbf{s}'$ has a degenerate distribution for Standard bias, since $\kinMat'$ is singular, but this is not problematic as long as $\mathbf{s}' + \boldsymbol{\epsilon}$ is non-degenerate, whose total covariance $(\sigma')^2 \kinMat' + \sigma^2_\epsilon \mathbf{I}$ is invertible as long as $\sigma^2_\epsilon \ne 0$.)
...
Therefore, after matching the variance components $\sigma^2$ and $(\sigma')^2$ as above, the random effect $\mathbf{s}'$ of the biased kinship matrix differs from the random effect $\mathbf{s}$ of the original kinship only by $\mathbf{1} s$, a difference compensated for by adjusting the intercept coefficient in \cref{eq:lmm_gwas}: $\alpha' = \alpha + \mathbf{1} s$.
No other regression coefficients, or the total residuals, change when \kinMat is replaced with $\kinMat'$, including the association coefficient $\beta_i$ that is the focus of the test.
...
Based on the PCA model of \cref{eq:pca_gwas,eq:kin_evd}, let $\mathbf{U}_d$ be the top eigenvectors of \kinMat, and $\mathbf{U}_d'$ those of $\kinMat'$.
...
\begin{appendices}
  ...
  Starting from a positive-definite kinship matrix \kinMat, we prove that WG-bias transformed matrix $\kinMat' = F^\text{WG}( \kinMat )$ is also positive definite (and therefore invertible), except in one degenerate case.
  ...
  Now consider two linear subspaces of $\mathbb{R}^n$, $S_1$ spanned by $\mathbf{1}$ and $S_2$ its complement (orthogonal to $\mathbf{1}$), and prove that $\kinMat'$ is positive definite in both subspaces, therefore it is positive-definite in the direct sum of the subspaces, which equals the entire space: $\mathbb{R}^n = S_1 \oplus S_2$.
  (This follows since vectors $\mathbf{v}$ for which $\kinMat'$ is not positive definite, if they exist, span a linear subspace, but its intersection to $S_1$, $S_2$, and therefore $S_1 \oplus S_2$, is trivial \citep{hefferon_linear_2020}.)
  In both subspaces we will prove that $\mathbf{v} \in S_i$ and $\mathbf{v} \ne \mathbf{0}$ implies $\mathbf{v}^\intercal \kinMat' \mathbf{v} > 0$ which proves that $\kinMat'$ is positive definite in that subspace.
  ...
  \mathbf{v}^\intercal \kinMat' \mathbf{v}
  =
  \frac{ v^2 }{1-\tilde{\varphi}}(\boldsymbol{1^\intercal \kinMat 1} - \tilde{\varphi} n^2)
  =
  \frac{ v^2 n^2 }{1-\tilde{\varphi}}(\bar{\varphi}-\tilde{\varphi})
  ,
  $$
  ...
  Note that $v^2, n^2, 1 - \tilde{\varphi} > 0$, so the desired result follows if $\tilde{\varphi} < \bar{\varphi}$, which is proven in \cref{sec:mean_kinship_ineqs}.
  In general it is true that $\tilde{\varphi} \le \bar{\varphi}$, and $\tilde{\varphi} = \bar{\varphi}$ occurs if and only if the kinship matrix has the degenerate form $\kinMat = \bar{\varphi} \mathbf{J}$, which is a singular matrix not expected in practice (in this case $\kinMat'$ is a matrix full of zeroes).
