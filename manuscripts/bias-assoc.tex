...
In contrast, the \textit{mean-of-ratios} (MOR) version (introduced here) has $w_i = (\pith (1-\pith))^{-1}$, so it upweighs rare variants; although it has no closed-form limit, it is approximately unbiased as well (\cref{sec:popkin_w_justif}) and it is connected to the most common standard estimator (explained below).
...
Used to calculate GCTA kinship estimates.
We pass $2 \kinMat$ for all kinship matrices tested.
...
To further differentiate methods, we increase noise in kinship estimation by purposefully simulating a small number of loci: $m=10,000$.
...
\begin{appendices}
  ...
  The original formulation had $w_i=1$ \citep{ochoa_estimating_2021}.
  ...
  \begin{align*}
    ...
    &=
      ...
      .
  \end{align*}
  Therefore, as before all the unknowns \pit and now also the weights $w_i$ collapse into a single parameter $v_m$, which is estimated under the original assumption that the minimum kinship is zero, giving $\AMinHat = -v_m$, so that
  $$
  ...
  ,
  \end{align*}
  where the expectation for $\pith (1-\pith)$ was also calculated previously \citep{ochoa_estimating_2021}.
  ...
  \begin{align*}
    ...
    &+ \left( \left( \frac{n_i}{n} \right)^2 -  I_{ij} I_{ik} \right) 4 \pith^2
      ...
      .
  \end{align*}
  To arrive at the desired result of $I_{ij} I_{ik} (\xij - 2 \pith) (\xij[k] - 2 \pith )$, it is necessary for the rest of the terms to vanish for arbitrary values of \pith, \xij, and \xij[k].
