...
\begin{align}
  ...
  \mathbf{s}
  &\sim
    \text{Normal} \left( \mathbf{0}, \sigma^2 \kinMat \right), \\
  \label{eq:pca_gwas}
  \text{PCA:}\quad
  \mathbf{y}
  &=
...
$\sigma^2$ is the random genetic effect variance component,
$d$ is the number of principal components,
$\mathbf{U}_d$ is the $n \times d$ matrix of top eigenvectors of \kinMat (often refered to as ``principal components'' in genetics),
...
Our empirical observations suggested that replacing a kinship matrix with its Standard biased version does not alter association statistics; here we provide proof of this fact.
...
This is fortunate, as the intercept and random effects or PCs coefficients are unreported nuisance parameters, while the focal genetic association coefficient and its p-value are unchanged by this bias.

Consider a kinship matrix \kinMat and its Standard bias-transformed version $\kinMat' = F^\text{std}( \kinMat )$ from \cref{eq:kinship_std_lim}.
The two kinship matrices are related more succinctly as
\begin{equation}
  ...
  ,
\end{equation}
where $\mathbf{C}$ is the $n \times n$ centering matrix.

\subsubsection{Proof for LMM case}

Consider a random effect $\mathbf{s}$ drawn from the original kinship matrix \kinMat, as given in \cref{eq:lmm_rand_eff}.
Using the affine transformation property of Multivariate Normal distributions, which despite its name also holds for rank-deficient matrices such as $\mathbf{C}$ (note $\mathbf{C}\mathbf{1}=\mathbf{0}$), then $\mathbf{C} \mathbf{s}$ has mean zero and covariance $\sigma^2 \mathbf{C} \kinMat \mathbf{C}$, so
$$
\mathbf{s}'
=
\mathbf{C} \mathbf{s}
...
\text{Normal} \left( \mathbf{0}, (\sigma')^2 \kinMat' \right),
$$
where $(\sigma')^2 = ( 1 - \bar{\varphi} ) \sigma^2$.
(This $\mathbf{s}'$ has a degenerate distribution, since $\kinMat'$ is singular, but this is not problematic as long as $\mathbf{s}' + \boldsymbol{\epsilon}$ is non-degenerate, whose total covariance $(\sigma')^2 \kinMat' + \sigma^2_\epsilon \mathbf{I}$ is non-singular as long as $\sigma^2_\epsilon \ne 0$.)
Note that
$
\mathbf{C} \mathbf{s}
=
\mathbf{s} - \mathbf{1} \bar{s}
,
$
where $\bar{s} = \frac{1}{n} \mathbf{1}^\intercal \mathbf{s}$ is the mean value of the elements of $\mathbf{s}$, so
$$
\mathbf{s}' = \mathbf{s} - \mathbf{1} \bar{s}
$$
are equal in distribution, where $\bar{s}$ has mean zero and variance $\sigma^2 \frac{1}{n^2} \mathbf{1}^\intercal \kinMat \mathbf{1}$, or in other words
$$
\bar{s} \sim \text{Normal} \left( 0, \sigma^2 \bar{\varphi} \right).
$$
Therefore, after matching the variance components $\sigma^2$ and $(\sigma')^2$ as above, the random effect $\mathbf{s}'$ of the biased kinship matrix differs from the random effect $\mathbf{s}$ of the original kinship only by $\mathbf{1} \bar{s}$, a difference that can be compensated for simply by adjusting the regression intercept coefficient $\alpha$ in \cref{eq:lmm_gwas} accordingly.
No other regression coefficients, or the total residuals, change when \kinMat is replaced with $\kinMat'$, including the association coefficient $\beta$ that is the focus of the test.
...
Based on the PCA model of \cref{eq:pca_gwas,eq:kin_evd}, let $\mathbf{U}_d$ be the eigenvector matrix of \kinMat, and $\mathbf{U}_d'$ that of $\kinMat'$
They key approximation is that
$$
\mathbf{U}_d' \approx \mathbf{C} \mathbf{U}_d,
$$
which is not strictly equal (note that $\mathbf{C} \mathbf{U}$ is not orthogonal, as eigenvectors must be), but we have found it to be a good approximation in practice.
In this case the eigenvector coefficients need not change, $\boldsymbol{\gamma}_d' = \boldsymbol{\gamma}_d$, since the difference in scale of the kinship matrices (the $\frac{1}{1 - \bar{\varphi}}$ factor of \cref{eq:kin-std-lim-C}) is entirely absorbed by the eigenvalues $\mathbf{\Lambda}'$, which are not used in the regression model.
As before, note that
$$
\mathbf{U}_d' \boldsymbol{\gamma}_d'
=
\mathbf{C} \mathbf{U}_d \boldsymbol{\gamma}_d
=
\mathbf{U}_d \boldsymbol{\gamma}_d - \mathbf{1} \bar{q},
$$
where
$$
\bar{q} = \frac{1}{n} \mathbf{1}^\intercal \mathbf{U}_d \boldsymbol{\gamma}_d.
$$
Therefore, as was the case for LMM, here the fixed PCA effects differ only by $\mathbf{1} \bar{q}$, which is compensated for by adjusting the intercept $\alpha$ accordingly, so the regression coefficient $\beta$ and the residuals are the same in both cases.
