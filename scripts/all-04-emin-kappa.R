...
par( mfrow = c(1,2) )
par( oma = c(1, 0, 0, 0) )
boxplot_data( data_sim, main = 'Admixed Family sim.', cut = cut_evs, ylab = 'Proportion non-PSD' )
panel_letter('A', adj = -0.2)
boxplot_data( data_real, main = '1000 Genomes', is_sim = FALSE, cut = cut_evs, ylab = 'Proportion non-PSD' )
panel_letter('B', adj = -0.2)
mtext( 'Matrix type, Kinship Estimate', side = 1, outer = TRUE )
fig_end()
