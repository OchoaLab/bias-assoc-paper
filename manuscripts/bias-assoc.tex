...
\begin{abstract}
  Common genetic association studies for structured populations, including Principal Component Analysis (PCA) and Linear Mixed-effects Models (LMM), model the correlation structure between individuals using population kinship matrices.
...
\begin{figure}[bp!]
  ...
  \caption{
    ...
    The association p-value vector (one value per tested locus) produced by each combination of association model (LMM vs PCA) and kinship matrix (x and y axes) was used to compute fractions of loci with absolute differences under 0.01 (color).
...
A more stringent criterion is the fraction of loci with p-values within 0.01 of each other, which is an approximate measure of agreement, and reveals few such agreements with LMMs except between estimators of the same type (\cref{fig:pvals_eq_sim}).
The fractions of agreement mirror our previous observations based on $\auc$, generally showing more agreement between all PCA p-values except for popkin MOR, for example.
Surprisingly, we find fractions at or near 1 for many estimators of different bias types (matched otherwise), particularly in LMMs.
